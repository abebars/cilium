...
func (s CIDRSlice) GetAsEndpointSelectors() EndpointSelectorSlice {
	...
	// We want to add the world selector whenever possible as it limits the growth of the
	// ipcache.
	// Check, dualstack
	if (option.Config.EnableIPv4 && option.Config.EnableIPv6 && hasIPv4AllBeenAdded && hasIPv6AllBeenAdded) ||
		// ipv4
		(option.Config.EnableIPv4 && !option.Config.EnableIPv6 && hasIPv4AllBeenAdded) ||
		// ipv6
		(!option.Config.EnableIPv4 && option.Config.EnableIPv6 && hasIPv6AllBeenAdded) {
